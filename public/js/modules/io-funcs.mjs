...
const calcYearBalance = () => {
    let yearIncms = 0, yearOutcms = 0, prcntg = 0;

    for (let io of global.iosArr) { //Se obtienen los ingresos y egresos anuales
        if (io.type === 'ingreso') {
            yearIncms += io.amount;
            continue;
        }
...
const calcMonthBalance = month => {
    ...
    for (let io of global.iosArr) { //Por cada inoutcome
        if (io.month == month) { //Si el mes es igual al que se desea calcular
...
const createIoHtml = (ioObj) => {
    ...
    ioObj.files.length && inoutcomeCnt.querySelector('.file-btn').addEventListener('click', () => dbFuncs.showDocsPrev(dbFuncs.getIoFromArr(ioObj.id))); //Si tiene archivos
...
const filterByMonthName = monthName => {
    for (let mnth of global.cntMonths.children) { //Por cada mes del DOM
        if (monthName !== mnth.id) { //Si no es el mes por el que se filtro
...
const searchOnIos = (keyWord) => {
    ...
    const ifMatchBrdr = global.prevTheme === 'dark' ? '#FCA311' : '#14213D';
    const ifNotMatchBrdr = global.prevTheme === 'dark' ? 'transparent' : '#14213D';
...
const repeatIoInMonths = (months, io) => {
    ...
    for (let month of months) { //Para cada mes en el que se tiene que agregar el io
        ...
        const newIo = { //Se crea un nuevo io con el nuevo mes y nueva fecha
            ...io,
...
const getMonthsToRepeat = (repeatEach, ioToRepeat) => {
    ...
    for (monthToStart; monthInNum < 12; monthInNum+=Number(repeatEach)) { //A partir del mes del io, mientras aun hayan meses se le suma la frecuencia de repeticion que selecciono el usuario
...
export { calcYearBalance, calcMonthBalance, createIoHtml, filterByMonthName, filterShowAll, searchOnIos, getParsedIoFiles, maxFileSizeReached, repeatIoInMonths, getMonthsToRepeat };